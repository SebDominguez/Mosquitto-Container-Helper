...
function createUser(){
    ...
    docker exec -it mosquitto mosquitto_passwd -b -c $CONTAINER_PASSWORD_FILE $username $password 
}
...
function listUsers(){
    ...
    docker exec -it mosquitto /usr/bin/cut -d ':' -f 1 "$CONTAINER_PASSWORD_FILE"
}

function deleteUser(){
    docker exec -it mosquitto mosquitto_passwd -D $CONTAINER_PASSWORD_FILE $1
    # Check the exit code
    if [[ $? -ne 0 ]]; then
